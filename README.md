...
of clonal barcodes from both NGS and scRNAseq data.scalable reproducible analysis workflows. 
...
<p> <img src="Nextclone_diagram_v4.png" width="500"/> </p>

The pipeline comprises two distinct workflows, one for NGS data and the other for scRNAseq data. 
...
nextflow run phipsonlab/Nextclone \
    ...
    --ngs_fastq_files data/ngs_fastq_files/
```
...
* `--mode` tells NextClone to run the workflow for NGS data.
...
* `--ngs_fastq_files` the location of the FASTQ files containing your raw NGS data.
...
* `mode`: tells NextClone to run the workflow for NGS or scRNAseq data.
* `publish_dir`: specifies the directory to copy the intermediate files and the final output to.
* `clone_barcodes_reference`: the location of the text file containing the list of reference clone barcodes to map the reads to.
